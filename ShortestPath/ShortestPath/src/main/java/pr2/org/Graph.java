    ...
    */
    public List<V> shortestPath(V v1, V v2) throws Exception{
		...
		while(!cola.isEmpty() && !encontrado){
			    ...
			    V v = cola.poll();
            //si el vertice que esta al principio de la cola es igual que v2, has encontrado el camino
			    encontrado = v.equals(v2);

			if (!encontrado){
				for (V s : obtainAdjacents(v)){
                    //metes los vertices adyacentes en la cola y en la traza 
        ...
        if(encontrado){
            ...
            while(v1 !=v2){
                camino.add(0,traza.get(v2));//empiezas a hacer el camino del reves, es decir, empezando por el ultimo vertice
                // devuelve el siguiente vértice del camino
                v2 = traza.get(v2);
            }
